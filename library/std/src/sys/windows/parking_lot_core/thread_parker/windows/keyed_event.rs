...
use crate::sys::c::{
    TRUE, ULONG, NTSTATUS, STATUS_SUCCESS, STATUS_TIMEOUT, ACCESS_MASK, BOOLEAN, GENERIC_READ, GENERIC_WRITE, HANDLE, LARGE_INTEGER, LPCSTR, LPHANDLE, PLARGE_INTEGER, LPVOID,
    CloseHandle, GetModuleHandleA, GetProcAddress,
};
...
impl KeyedEvent {
    ...
    pub unsafe fn park_until(&'static self, key: &AtomicUsize, timeout: Instant) -> bool {
        ...
        let mut nt_timeout: LARGE_INTEGER = match value {
            Some(x) => x,
            None => {
